(ns Projects.own.advent-of-code-2019.day3
...
(defn read-wire [wire]
  (map read-instruction (str/split wire #",")))

(def input
  (let [wires (str/split raw-input #"\n")]
    (map read-wire wires)))
  ...
  (let [m (meta coord)
    ...
    (with-meta
      (case dir
        ...
        "U" [x (inc y)]
        "D" [x (dec y)])
      (update m :index (fnil inc 0)))))

(defn expand-instruction [[dir n]]
  (repeat n dir))

(defn expand [wire]
  (mapcat expand-instruction wire))

(defn coords [wire]
  (rest
   (reductions move [0 0] (expand wire))))

(defn distance [[x y]]
  (+ (Math/abs x)
     (Math/abs y)))

(defn step-intersection [s1 s2]
  (reduce (fn [intersections s]
            (if (contains? s2 s)
              (conj intersections (with-meta s {:index (+ (:index (meta s))
                                                          (:index (meta (get s2 s))))}))
              intersections)) #{} s1))
