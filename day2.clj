(ns Projects.own.advent-of-code-2019.day2)

(def input
  [1,0,0,3,1,1,2,3,1,3,4,3,1,5,0,3,2,1,10,19,1,19,5,23,2,23,9,27,1,5,27,31,1,9,31,35,1,35,10,39,2,13,39,43,1,43,9,47,1,47,9,51,1,6,51,55,1,13,55,59,1,59,13,63,1,13,63,67,1,6,67,71,1,71,13,75,2,10,75,79,1,13,79,83,1,83,10,87,2,9,87,91,1,6,91,95,1,9,95,99,2,99,10,103,1,103,5,107,2,6,107,111,1,111,6,115,1,9,115,119,1,9,119,123,2,10,123,127,1,127,5,131,2,6,131,135,1,135,5,139,1,9,139,143,2,143,13,147,1,9,147,151,1,151,2,155,1,9,155,0,99,2,0,14,0])

{:pos 0
 :ints ints}

(defn read-next [state]
  [(nth (:ints state) (:pos state))
   (update state :pos inc)])

(defn add [state]
  (let [[input1 state1] (read-next state)
        [input2 state2] (read-next state1)
        [output state3] (read-next state2)]
    (update state3 :ints #(assoc % output (+ (nth % input1) (nth % input2))))))

(defn mult [state]
  (let [[input1 state1] (read-next state)
        [input2 state2] (read-next state1)
        [output state3] (read-next state2)]
    (update state3 :ints #(assoc % output (* (nth % input1) (nth % input2))))))

(defn exec [state]
  (let [[opcode next-state] (read-next state)]
    (case opcode
      1 (exec (add next-state))
      2 (exec (mult next-state))
      99 state)))

(comment
  (exec {:pos 0
         :ints [1,9,10,3,2,3,11,0,99,30,40,50]})

  (exec {:pos 0
         :ints (-> input
                   (assoc 1 12)
                   (assoc 2 2))})

  (for [noun (range 0 100)
        verb (range 0 100)
        :let [output (-> {:pos 0
                          :ints (-> input
                                    (assoc 1 noun)
                                    (assoc 2 verb))}
                         exec
                         :ints
                         (nth 0))]
        :when (= output 19690720)]
    #_[noun verb]
    (+ (* 100 noun) verb)
  )
